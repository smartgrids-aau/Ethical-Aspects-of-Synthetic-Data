...
1. **Extract & downsample** (`01_extract_snm_archive.bat`, `02_downsample_synd.py`,
   `03_compute_snm_power.py`). SNM does not store direct power readings — whole-home
...
2. **Feature extraction** (`04_privacy_analysis.py`, `day_features`): each day of
...
All files are in the repository root (the scripts load their sample/input files from
the current working directory, so keeping everything together is required for the
code to run as-is):
...
├── synd_day_features.csv                   Extracted per-day features for SynD (output of steps 2-4)
...
The three `.csv` files are the small, already-extracted per-day *feature* tables
(outputs of the pipeline), **not** the raw smart meter datasets, so that
`04_privacy_analysis.py` can be run and checked end-to-end without first downloading
and reprocessing the full raw archives.
...
Clone or download this repository (keep all files together in one folder), then open
`04_privacy_analysis.py` and, instead of calling `extract(...)` on raw mains CSVs, load
`synd_day_features.csv` and `snm_all_features.csv` directly — they already have the
same column layout the pipeline produces. This lets you check the privacy metrics and
PRS computation immediately, without downloading the raw datasets.
...
python 03_compute_snm_power.py
...
same folder, and prints the same PRS and re-identification numbers reported above and
